...
candidate for all \(n\).  For \(n=4\) we prove the sharp lower bound
\(3072R^{11}\), an unconditional matrix-gradient upper bound, and a reduced
four-point Blaschke--Pick criterion for the sharp upper bound.  We also give
a reproducible local certificate for the only hard corner encountered in the
third Schur-pivot chart.
...
quartic matrices it proves the sharp lower bound \(3072R^{11}\), an
unconditional matrix-gradient upper bound, and a reduced four-point
Nevanlinna--Pick criterion for the sharp upper bound.  The graph,
...
rank-one defect model \(S_B\).  If the reduced Pick matrix is positive
semidefinite on the centered four-point disk, identity
\eqref{eq:reduced-pick-recursion} implies positivity of the original Pick
matrix \eqref{eq:quartic-pick-matrix}.  The finite Nevanlinna--Pick theorem and
...
\]
attained by \(tJ_n\).  The cases \(n=2,3\) are proved here.  For \(n=4\) this
formula is reduced to the four-point Pick positivity criterion of
Theorem~\ref{thm:tr0-4x4}; the formula remains conjectural for \(n\ge4\)
until the full finite certificate is supplied.
\end{conjecture}
...
\end{equation}
 ...
 records the finite certificate route and the local hard-corner closure, but
 the global chamber tables are not printed in this article.
\end{remark}
...
\begin{proposition}[Finite certificate route and the \(\gamma_3\) hard corner]
\label{prop:interior-machine-certificate}
The global reduced Pick positivity problem for centred non-confluent
quadruples is reduced to a finite list of rational Bernstein certificates.
Among the two-parameter \(3{+}1\) Schur-pivot certificates, the only direct
Bernstein hard corner is the \(\gamma_3\) corner
\[
  \rho\in[3/4,1],\qquad y\in[3/4,1],
\]
near \((\rho,y)=(1,1)\).  That local hard corner is closed by the blow-up
certificate in Appendix~\ref{app:gamma-three-hard-corner}.
\end{proposition}
...
boxes.  The present paper prints the resolved certificate for the only hard
corner found in the two-parameter \(3{+}1\) chart, namely the third Schur pivot
near
\[
  \rho\in[3/4,1],\qquad y\in[3/4,1],
\]
at the square endpoint \((\rho,y)=(1,1)\).
...
coefficients.  This proves the local hard-corner certificate.  The global
reduced Pick criterion is obtained once the remaining finite chamber tables
from Proposition~\ref{prop:finite-interior-reduction} are supplied.
\end{proof}

If the reduced Pick criterion \eqref{eq:reduced-pick-matrix} holds globally,
\[
  \|A_B\|_2
...
Proposition~\ref{prop:finite-interior-reduction}.  Proposition~\ref{prop:interior-machine-certificate}
closes the only hard \(\gamma_3\) corner of that finite problem, while the full
non-normal \(n=4\) sharp upper bound remains conditional on the remaining
finite chamber tables.
\end{remark}
...
\begin{remark}[A non-generic torus scalar example]\label{rem:T2-degenerate-three-zero}
...
This article supplies the resolved certificate for the unique hard corner,
Appendix~\ref{app:gamma-three-hard-corner}; the remaining regular chamber
tables are finite supplementary data required for an unconditional non-normal
quartic sharp upper bound.
\end{remark}
...
The author declares no competing interests.  No external funding was used
for this work.
