...
Remark~\ref{rem:T2-degenerate-three-zero}
...
\]
The Schur--Julia parametrisation of a neighbourhood of \(T\) in the contraction
ball gives analytic disks of the form
\[
...
for \(W\) has complex dimension at least two.  The trace constraint
\[
  \operatorname{tr}T(W)=0
\]
is one holomorphic scalar equation whose derivative at the origin is the
linear functional \(W\mapsto\operatorname{tr}(D_LWD_R)\).  If this functional
is non-zero, the implicit function theorem leaves a non-constant trace-zero
analytic disk through \(T\); if it is zero, any non-zero small one-parameter
direction in the kernel already gives such a disk to first order and the same
implicit-function argument applies after choosing a transverse parameter.
This contradicts complex extremality.  Hence
\[
  \operatorname{rank}D_L\,\operatorname{rank}D_R=1.
\]
For a square contraction the two defect ranks are equal, so a non-unitary
complex extreme point has defect indices \((1,1)\).
...
\[
\begin{array}{c|c|c|c}
\text{chart/stratum} & \text{pivots} & \text{method} & \text{equality faces}\\ \hline
K\text{-region/compact core} & \gamma_1,\gamma_2,\gamma_3
  & \text{finite Bernstein/Schur tables} & \text{none in the open cell}\\
\text{paired family} & \gamma_1,\gamma_2,\gamma_3
  & \text{symbolic factor/Sturm} & \text{radial square}\\
\text{boundary and confluent layers} & \text{all active pivots}
  & \text{boundary induction and confluent factors} & \text{lower strata}\\
\text{real boundary face} & \text{pair inequalities}
  & \text{CRT rational Bernstein} & \text{boundary faces}\\
3{+}1\text{ boundary layer} & \gamma_1,\gamma_2,\gamma_3
  & \text{Sturm/Bernstein plus blow-up} & \text{square/confluent}\\
\text{radial square} & \text{determinant ratio}
  & \text{explicit factorisation} & \rho=1\\
\text{one-dimensional family} & \text{reduced pivots}
  & \text{explicit positive factorisation} & \text{endpoints}\\
\gamma_3\text{ hard corner} & \gamma_3
  & \text{ridge blow-up and exact Bernstein} & \text{resolved faces}
\end{array}
\]
The hard-corner verifier hash is
\[
  \texttt{473ac0d3875c1c3989ff42f1291b8db4ee104b3e4769283fdc1c754a5568dabc}.
\]
