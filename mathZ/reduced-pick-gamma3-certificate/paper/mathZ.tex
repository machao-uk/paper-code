...
\(3072R^{11}\): the non-normal rank-one defect case is closed by a finite
four-point Blaschke--Pick certificate and a blow-up resolution of its only
hard Bernstein corner.
...
combinatorial spectral topology; Nevanlinna--Pick interpolation; certified computation.
...
quartic matrices it proves the sharp constant \(3072R^{11}\).  The non-normal
quartic upper bound is reduced to, and closed by, the four-point
Nevanlinna--Pick certificate.  The graph,
...
Propositions~\ref{prop:T2-no-puncture}--\ref{prop:T2-two-punctures}
...
constants exactly for $n=2$ and $n=3$ (traceless), gives the current
traceless $n=4$ bounds and reduction, and states the general conjecture.  The cubic examples in
Sections~\ref{sec:nonabelian-cubic} and~\ref{sec:EP3} use the exact constant
$162R^5$; the quartic example in Section~\ref{sec:local-models} uses the $n=4$ bounds.
...
$\nabla_H\Disc=4B^T$.  Von Neumann gives
$|D\Disc(H)[\delta H]|\le4\|B\|_*\|\delta H\|_2\le8\|H\|_2\|\delta H\|_2\le8R\|\delta H\|_2$.
...
\]
Consequently the lower bound $\cG_n^{\tr=0}(R)\ge n(n-1)|\Disc(J_n)|R^{n(n-1)-1}$ gives
...
\begin{theorem}[Traceless $4\times4$ bounds and reduced sharpness criterion]\label{thm:tr0-4x4}
Let $\cT_4(R)=\{H\in M_4(\bbC):\tr H=0,\|H\|_2\le R\}$.
\[
  3072R^{11}\le \cG_4^{\tr=0}(R)\le \frac{60416}{3}R^{11}.
\]
The lower bound is attained in the radial limit by \(tJ_4\),
\(J_4=\diag(1,\ii,-1,-\ii)\).  If the reduced four-point Pick matrix
\[
  \left[
  \frac{1-r_i\overline{r_j}}{1-\lambda_i\overline{\lambda_j}}
  \right]_{i,j=1}^4,\qquad
  r_i=768^{-1}\bigl(c_1+c_2\lambda_i+c_3\lambda_i^2\bigr),
\]
is positive semidefinite for every centred
\(\lambda_j\in\overline{\mathbb D}\), then the sharp value is
\[
  \cG_4^{\tr=0}(R)=3072R^{11}.
\]
\end{theorem}
...
unit-circle and mixed-stratum reductions.  For non-normal matrices, Schur
triangularisation and the finite Blaschke model reduce the problem to the
rank-one defect model \(S_B\).  By
Proposition~\ref{prop:interior-machine-certificate}, the reduced Pick matrix
is positive semidefinite on the centered four-point disk.  Identity
\eqref{eq:reduced-pick-recursion} therefore implies positivity of the original
Pick matrix \eqref{eq:quartic-pick-matrix}.  The finite Nevanlinna--Pick theorem and
Sarason's lifting theorem then give
\(\|c_1S_B+c_2S_B^2+c_3S_B^3\|_2\le768\), hence
\[
  \|c_1S_B+c_2S_B^2+c_3S_B^3\|_*
  \le4\|c_1S_B+c_2S_B^2+c_3S_B^3\|_2
  \le3072 .
\]
...
is $3072$, attained at $\theta=\pi/2$ (i.e.\ $u=\pm\ii$), which is the square
...
We now pass from normal contractions to arbitrary traceless contractions.  The
argument uses the complex extreme boundary of the trace-zero contraction ball.
For fixed trace-zero direction, the map
\(H\mapsto A(H)=c_1H+c_2H^2+c_3H^3\) is holomorphic and
\(\|A(H)\|_*\) is plurisubharmonic, since
\[
  \|A(H)\|_*=\sup_{\|Y\|\le1}|\operatorname{tr}(Y^*A(H))|.
\]
Hence its maximum on the compact trace-zero contraction ball is attained on
the complex extreme boundary.

Let \(T\) be such a complex extreme contraction in Schur form.  If
\(D_L=(I-TT^*)^{1/2}\) and \(D_R=(I-T^*T)^{1/2}\), then every sufficiently
small perturbation of the form \(D_LZD_R\) keeps \(T\) inside the contraction
ball to first order, and the standard Julia-operator parametrisation makes this
an actual analytic disk.  The trace-zero slice removes only the single complex
linear condition
\[
  \operatorname{tr}(D_LZD_R)=0.
\]
Therefore, if \(\operatorname{rank}D_L\operatorname{rank}D_R\ge2\), there is a
non-zero trace-zero analytic disk through \(T\), contradicting complex
extremality.  Since \(T\) is square, \(\operatorname{rank}D_L=\operatorname{rank}D_R\).
Thus a non-unitary complex extreme point has defect rank one:
\[
  \operatorname{rank}(I-T^*T)=\operatorname{rank}(I-TT^*)=1.
\]
The unitary extreme points are normal and have already been covered by the
all-boundary normal estimate.

A finite-dimensional completely non-unitary contraction with defect indices
\((1,1)\) is unitarily equivalent to the compressed shift \(S_B\) associated
with a finite Blaschke product
\[
...
\end{equation}
 ...
 quartic sharp upper bound; this semidefiniteness is proved in
 Proposition~\ref{prop:interior-machine-certificate}.
\end{remark}

\begin{lemma}[Reduced Pick recursion]\label{lem:reduced-pick-recursion}
By the finite Nevanlinna--Pick theorem, \(\Pi_\lambda\succeq0\) is equivalent
to the existence of a Schur function \(f\in H^\infty(\bbD)\),
\(\|f\|_\infty\le1\), with
\[
  f(\lambda_j)=q_\lambda(\lambda_j),\qquad j=1,\ldots,4.
\]
Sarason's commutant lifting theorem would then give
\[
  \|q_\lambda(S_B)\|_2
  =
  \inf\{\|f\|_\infty:\ f(\lambda_j)=q_\lambda(\lambda_j)\}\le1.
\]
Thus Remark~\ref{conj:pick-blaschke-certificate} is exactly the remaining
finite certificate needed for the non-normal sharp \(n=4\) upper bound.
...
\[
  ...
  \right]_{a,b=1}^\ell
\]
is positive semidefinite.  Then the Pick matrices for the radially perturbed
...
interior Pick matrix plus an \(o(1)\) perturbation.  If the interior matrix is
positive definite, positivity follows for \(t\) close to \(1\).  The
semidefinite case follows by replacing the interior block by its
\(\varepsilon I\) regularisation and letting \(\varepsilon\downarrow0\).
\end{proof}
...
The Schur complement with respect to this \(3\times3\) block tends to the
interior scalar \((1-|r(-r)|^2)/(1-r^2)\ge0\).  Hence the approximating Pick
matrices are positive semidefinite for \(t\) close to \(1\), and the boundary
certificate follows by continuity.  The only point where a boundary first
...
\]
Thus the remaining certificate may be stated without determinants: prove
\[
...
\begin{proposition}[Analytic certificate for the interior reduced Pick criterion]
...
\(\lambda_1,\lambda_2,\lambda_3,\lambda_4\in\overline{\mathbb{D}}\).
\end{proposition}
...
boxes.  All non-corner boxes are certified by the Bernstein tables described
in Appendix~\ref{app:pick-certificate}.  The only box on which the direct
Bernstein expansion has negative coefficients is the third Schur-pivot
hard corner in the two-parameter \(3{+}1\) chart,
\[
...
Consequently every Schur pivot polynomial in the finite interior reduction is
non-negative, with zeros only on the already certified boundary or confluent
faces.  On the non-confluent interior the pivots are strictly positive, hence
the Schur recursion gives \(\Pi^{\mathrm{red}}_\lambda\succ0\).  Taking limits
gives positive semidefiniteness on the closed non-confluent locus.
\end{proof}

Since the reduced Pick criterion \eqref{eq:reduced-pick-matrix} holds globally
by Proposition~\ref{prop:interior-machine-certificate},
\[
  \|A_B\|_2
...
Proposition~\ref{prop:radial-square-model}.  The final step---positive
semidefiniteness of the reduced Pick matrix on the genuinely interior,
non-confluent cells---is established by the finite Bernstein and blow-up
certificate in
Proposition~\ref{prop:interior-machine-certificate}.
\end{remark}
...
\end{enumerate}
If a perturbation satisfies the radius in Theorem~\ref{thm:sampled-rep},
then the same generator list $(b_1,\ldots,b_m)$ is certified for the
perturbed family.
\end{proposition}
...
are therefore disjoint, so no collision can occur.  Homotopy invariance in
configuration space gives the braid statement.
\end{proof}
...
every eigenvalue of \(H(x)\) in the union of these disks.  Since there are
exactly \(n\) eigenvalues counted with multiplicity and the disks are
disjoint, each disk contains exactly one eigenvalue of \(H(x)\).  This gives
...
\(-1\) after one positive circuit and the local pair makes the positive
half-twist of positions \(1\) and \(2\).  This is \(\sigma_1\):
\[
\begin{array}{c|c|c}
...
\(|\Delta(H_t(k))-\Delta(H(k))|\le\cG_\cS(R_*)\sup\|\widetilde H-H\|_2
<M_{\bbT^2}/2\),
...
Explicit computations appear in
Propositions~\ref{prop:T2-no-puncture}--\ref{prop:T2-two-punctures} below.
...
\]
The image is $\langle(\sigma_1\sigma_2)^3\rangle\cong\bbZ$, a cyclic
subgroup of $Z(B_3)$.  Theorem~\ref{thm:torus-full} applies with
$\cG_{\tr=0,3}(R_*)=162R_*^5$.
\end{proposition}

\begin{proposition}[Punctured-torus monodromy, two exceptional points]
\label{prop:T2-two-punctures}
Keep $H=gM$ with
\[
  g(k_1,k_2)=e^{\ii k_1}+0.3e^{-\ii k_1}+0.5e^{\ii k_2}-0.8.
\]
The equation $g=0$ on $\bbT^2$ has exactly two transverse zeros, located at
\begin{align*}
...
\]
The fundamental group $\pi_1(\bbT^2\setminus\{p_1,p_2\})\cong F_3$ has
presentation $\langle a,b,\mu_1,\mu_2\mid[a,b]\mu_1\mu_2=1\rangle$.
Setting $\Omega=(\sigma_1\sigma_2)^3$, the braid monodromy representation
$\beta:\pi_1(\bbT^2\setminus\{p_1,p_2\})\to B_3$ is
\[
  \beta(a)=\Omega,\quad\beta(b)=1,\quad
  \beta(\mu_1)=\Omega^{-1},\quad\beta(\mu_2)=\Omega.
\]
The image is $\langle\Omega\rangle\cong\bbZ\subset Z(B_3)$.
\end{proposition}
...
This zero therefore carries no braid monodromy and is excluded from the
exceptional-point count.
...
\emph{Braid monodromy}.
Since $H=gM$ and the eigenvalues of $M=\diag(1,\omega,\omega^2)$ are
distinct and fixed, all three eigenvalues of $H$ rotate rigidly by $g$.
By the same winding-number argument as in
Proposition~\ref{prop:T2-no-puncture}, the torus generators give
$\beta(a)=\Omega$ and $\beta(b)=1$.
For the meridian $\mu_j$ around $p_j$, the local degree of $g$ at $p_j$
equals $\sgn J(p_j)$; a winding-number-$w$ loop of $g$ produces
braid $\Omega^w\in Z(B_3)$.  Hence $\beta(\mu_1)=\Omega^{-1}$ and
$\beta(\mu_2)=\Omega$.

The Zariski--van Kampen relation $\beta([a,b])\beta(\mu_1)\beta(\mu_2)=1$
reduces to $[\Omega,1]\cdot\Omega^{-1}\cdot\Omega=1$, which holds since
$\Omega\in Z(B_3)$.  This is consistent: the abelian image forces all
ZvK relations to be automatic, reflecting that the representation factors
through $\pi_1^{\rm ab}\cong H_1(\bbT^2\setminus\{p_1,p_2\})$.

\emph{Contrast with the companion cubic}
(Section~\ref{sec:nonabelian-cubic}): for $E^3-3E-k$, the meridians map to
$\sigma_1,\sigma_2\notin Z(B_3)$ and the image is all of $B_3$.
Here every generator maps into the centre because the $H=gM$ structure
propagates the winding of the scalar factor $g$ to all strands simultaneously.
\end{proof}
...
\end{array}
\]
Thus the brute-force table is a legitimate finite verification problem, but
not a practical appendix-level proof.  The proof above uses the second option:
the lower-dimensional Schur-pivot certificate isolates the radial-square
degeneration, and Appendix~\ref{app:gamma-three-hard-corner} resolves the only
hard Bernstein corner by a blow-up.
\end{remark}

\begin{remark}[Why this is not an infinite regress]\label{rem:not-infinite-regress}
The reduction stops at this point.  No new analytic or functional-analytic
statement remains after Proposition~\ref{prop:finite-interior-reduction}: the
objects to be checked are explicitly generated polynomials with rational
coefficients on rational boxes.  The required certificates are supplied here:
the regular boxes are covered by the Schur-pivot Bernstein tables, and the
unique hard corner is covered by the blow-up certificate in
Appendix~\ref{app:gamma-three-hard-corner}.  Therefore the non-normal quartic
case is closed, rather than merely reduced to a further condition.
\end{remark}
...
For auditability, the exact script and output used to regenerate the numerical
statements above are
\[
\begin{gathered}
\texttt{/Users/ray/Downloads/MathZ\_gamma3\_hard\_corner\_certificate.py},\\
\texttt{/Users/ray/Downloads/MathZ\_gamma3\_hard\_corner\_certificate\_output.md}.
\end{gathered}
\]
...
\]
From $re^{i\alpha}+\rho e^{i\beta}=-1$ we get
$r^2+\rho^2+2r\rho\cos(\alpha-\beta)=1$, so $r\rho|\cos(\alpha-\beta)|\le\tfrac12$.
Combining these constraints gives $|1-ab|\le 1-r^2\rho^2=(1-r\rho)(1+r\rho)$.
...
The author(s) declare no competing interests.  No external funding was used
for this work.
